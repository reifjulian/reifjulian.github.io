...
I've made the following custom adjustments:
  ...
  - Overrode `/_includes/head.html` to pin Font Awesome to a fixed version (the theme loads `@latest`)
  ...
  - Added SEO settings to `_config.yml` (`og_image`, `social` profile links for schema.org structured data, per-page `description` front matter)
